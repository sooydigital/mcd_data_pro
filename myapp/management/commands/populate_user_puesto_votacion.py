...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        VotantePuestoVotacion

        for puesto in PUESTOS:
            clean_puesto = puesto[0].strip()
            puesto_votacion = PuestoVotacion.objects.filter(name=clean_puesto).first()
            if puesto_votacion:
                num_intencidad = random.randint(10, 50)
                for i in range(num_intencidad):
                    mesa = random.randint(0, 9)
                    votante_puesto_votacion = VotantePuestoVotacion.objects.create(
                        votante=votante,
